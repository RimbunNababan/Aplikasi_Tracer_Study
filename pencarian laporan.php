...
$per_hal = 3;
$jumlah_record = mysqli_query($koneksi, "SELECT * from laporan_dana");
...
if (isset($_GET['cari'])) {
  ...
  $query = "SELECT * FROM laporan_dana WHERE Nama_laporandana LIKE '%$cari%'";
  $laporan_dana = mysqli_query($koneksi, $query);
  $count = mysqli_num_rows($laporan_dana);
  ...
  if ($count == 0) {
    ...
    $no = 1;
    while ($b = mysqli_fetch_array($laporan_dana)) {
      echo '
      <table>
            <tr>
          
         
          <td>' . $b['Nama_laporandana'] . '</td>
          <td>
            <img src="image/dana.jpg" alt="Image" width="150" height="60">
          </td>
          <td>
  ...
  $laporan_dana = mysqli_query($koneksi, "SELECT * FROM laporan_dana ORDER BY id_laporan DESC LIMIT $start, $per_hal");
